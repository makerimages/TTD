...
public class GuiButton
{
    ...
    public void draw(Graphics g)
    {
        ...
        image.draw(this.x, this.y, this.w * 2, this.h * 2);

        Color textColor = (enabled && !isHovering) ? Color.white : Color.gray;
        buttonFont.drawString(this.x + (this.w * 2 - buttonFont.getWidth(this.text)) / 2, this.y + buttonFont.getHeight(this.text) / 2, this.text, textColor);
    }

    private Image getDisplayImage()
    {
        if (!enabled)
            return disabled.getImage();
        if (isHovering)
            return hover.getImage();
        return notHover.getImage();
    }
}
